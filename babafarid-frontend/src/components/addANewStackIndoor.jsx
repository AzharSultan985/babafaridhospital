...
export default function AddANewStockIndoor() {
  const {  setListOFNewstack,AddNewstock_Indoor,  FetchlastMonthIndoorMed,
  LastMonthindoorMed } = useContext(AppContext);
//console.log("LastMonthindoorMed",LastMonthindoorMed);
  ...
  const AddANewStack = (index) => {
    ...
    if (newMedicine.Medname || newMedicine.company || newMedicine.expdate || newMedicine.quntity) {
  ...
  <table className="w-full text-gray-700">
          ...
          <tbody>
            {Array.isArray(LastMonthindoorMed) && LastMonthindoorMed.length > 0 ? (
              LastMonthindoorMed.map((data, index) => (
                <tr
                  key={data._id || index}
