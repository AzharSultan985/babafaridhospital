...
const RecepLogin = () => {
  ...
  const { recep_Login_Res,Recep_Login} = useReception();


  const [showPassword, setShowPassword] = useState(false);
  const [username, setUsername] = useState("");
  const [password,setPassword] = useState("");
const HandleRecep_User=async ()=>{
await Recep_Login(username,password)
}
  return (
    <>
      <section className="w-full flex justify-center bg-gray-50 h-[5vh] ">
        {recep_Login_Res && (
          ...
          >
            ...
            <div className="ps-4 text-sm font-normal">{recep_Login_Res}</div>
          </div>
        )}
      </section>
