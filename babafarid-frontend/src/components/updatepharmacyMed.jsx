...
export default function UpdateStockPharmacy() {
  const {FetchCurrentMonthPharmaMed,
currentMonthpharmacyMed,ListOfNewStock, setListOfNewStock,UpdateNewstock_Pharmacy} = usePharmacy();
  const inputRefs = useRef([]);
  const [selectedRows, setSelectedRows] = useState([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
// ////console.log(currentMonthpharmacyMed);
    ...
    <>
      ...
      <div className="rounded-lg shadow-md border border-gray-300 overflow-y-auto h-[70vh]">
        <table className="w-full text-gray-700">
          ...
          <tbody>
            {currentMonthpharmacyMed.length > 0 ? (
              currentMonthpharmacyMed.map((data, index) => (
                <tr
                  key={data._id || index}
